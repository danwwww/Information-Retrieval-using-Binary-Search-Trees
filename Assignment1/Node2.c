...
//if the tree 1 is empty,make tree2 as the root
    if(tree1==NULL){
        ...
        tree1->next_data=NULL;

    }
//if the weight of tree 2 is less to the weight of tree1, insert it to tree1's left child 
...
//if the key is duplicate, store the data into a 'linked list'
    else if(strcmp(tree2->Name,tree1->Name)==0){
        tree1->next_data=insertToTree(tree1->next_data,tree2);
    }
...
//if there are duplicate result with the same key, print it out too.
    if(aTree->next_data!=NULL){
        printNode(aTree->next_data,fp);
    }
